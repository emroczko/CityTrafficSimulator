/**
 * definitions.hpp
 * Configuration file containing path to every texture or font file using in application and some global values that we can change from here.
 */

#pragma once

#define SCREEN_WIDTH 1680
#define SCREEN_HEIGHT 1000

#define SIDEWALK_SIZE 6
#define ROAD_SIZE 18
#define ROAD_STRIPES_SIZE 3
#define ROAD_IMAGE_SIZE 51

#define STARTING_CELL_ROW -2
#define STARTING_CELL_COL 0

#define STARTING_CELL_ROW2 -2
#define STARTING_CELL_COL2 15

#define SPLASH_STATE_SHOW_TIME 0.1
#define SPLASH_SCENE_BACKGROUND_FILEPATH "Resources/background_splash.jpeg"

#define MENU_BACKGROUND_FILEPATH "Resources/background_menu.jpeg"

#define BUTTON_FILEPATH "Resources/button.png"

#define BUTTON_PRESSED_FILEPATH "Resources/button_pressed.png"

#define BACKGROUND_TEXTURE_FILEPATH "Resources/background_texture.jpg"

#define TEXT_FONT_FILEPATH "Resources/font.ttf"

#define SELECTED_CELL_TEXTURE "Resources/SelectedCell.png"

#define STREET_TEXTURE "Resources/street.png"

#define INTERSECTION_TEXTURE "Resources/intersect.png"

#define T_INTERSECTION_TEXTURE "Resources/T_intersect.png"

#define TURN_TEXTURE "Resources/turn.png"

#define CROSSING_TEXTURE "Resources/crossing.png"

#define ENTRY_TEXTURE "Resources/entry.png"

#define CAMERA_TEXTURE "Resources/camera.png"
